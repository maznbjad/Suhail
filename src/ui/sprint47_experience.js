...
(function () {
  ...
  function buildDiagnostic(path) {
    ...
    if(path==='qudrat') {
      ...
      const subjectMap=[['رياضيات','math'],['فيزياء','physics'],['كيمياء','chemistry'],['أحياء','biology']];
  ...
  function finishDiagnostic() {
    ...
    else weighted=avg(['رياضيات','فيزياء','كيمياء','أحياء'].map(x=>groups[x]?groups[x].percent:0));

...
(function(){
  ...
  function renderStages(subject){state.view='stages';state.subject=subject;const p=activate();if(!p)return;const rows=stages(subject).map(st=>{const label=String(st).replace(/(\d+[-–]\d+)$/,'\u2066$1\u2069');const pdf=window.SuhailPdfReader102?.getBook?.(st);if(pdf)return menu(label,'ملف مراجعة كامل قابل لتحديد النص والتظليل',`${pdf.pageCount} صفحة`,`stage::${encodeURIComponent(subject)}::${encodeURIComponent(st)}`);const us=units(subject,st),n=us.reduce((a,x)=>a+x.items.length,0);return menu(label,`${us.length} فصول أو وحدات مرتبة`,`${n} درس`,`stage::${encodeURIComponent(subject)}::${encodeURIComponent(st)}`)});p.innerHTML=shell(`${top(subject,'اختر الكتاب المناسب لك','tahsili')}<div class="s101-list-wrap"><div class="s101-list">${rows.join('')}</div></div>`);bindCommon(p);p.scrollTop=0;}

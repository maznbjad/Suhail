...
(function(){
  ...
  function decorateCurrent(){let q=null;try{q=activeQuestions?.[activeIndex]}catch(_){};const box=document.getElementById('questionSummaryLink');if(!box||!q)return;const r=refFor(q);if(!r){return}const {summary:s,block:b}=r;box.innerHTML='<button type="button" class="s69-knowledge-link"><span class="s69-kl-icon">↗</span><span class="s69-kl-copy"><b>'+esc(b.title)+'</b><small>'+esc(s.title+' • '+String(b.content||'').slice(0,85))+'</small></span><em>‹</em></button>';box.classList.remove('hidden');box.onclick=()=>openBlock(b.id)}
  ...
  function install(){ensureModal();patch();setInterval(patch,1000);new MutationObserver(()=>{patch();if(document.getElementById('questionCard'))decorateCurrent();if(document.getElementById('savedQuestionsPage')?.classList.contains('active'))renderSaved();renderSummaryKnowledge()}).observe(document.body,{subtree:true,childList:true,attributes:true,attributeFilter:['class']});setTimeout(()=>{decorateCurrent();renderSaved();renderSummaryKnowledge()},180)}
